...
def log(msg):
    sys.stderr.write(f"[rustup-plugin] {msg}\n")
    sys.stderr.flush()
...
def get_config_path() -> str:
    ...
    if userprofile and sys.platform == "win32":
        return os.path.join(userprofile, ".rustup", "settings.toml")
    return os.path.expanduser("~/.rustup/settings.toml")
...
def check_installed(args: dict, request_id: str) -> dict:
    installed = (
        shutil.which("rustup.exe") is not None
        or shutil.which("rustup") is not None
    )
    return {
