...
class TestScoopPlugin(unittest.TestCase):
    ...
    def test_get_config_path_with_xdg(self, mock_getenv):
        def side_effect(key):
            if key == "XDG_CONFIG_HOME": return "C:\\xdg"
            if key == "USERPROFILE": return "C:\\Users\\test"
            return None
    ...
    def test_get_config_path_fallback_userprofile(self, mock_getenv):
        def side_effect(key):
            if key == "XDG_CONFIG_HOME": return None
            if key == "USERPROFILE": return "C:\\Users\\test"
            return None
        mock_getenv.side_effect = side_effect
        
        path = plugin.get_config_path()
        self.assertEqual(path, os.path.join("C:\\Users\\test", ".config", "scoop", "config.json"))
    ...
    def test_write_json_uses_mkstemp(self, mock_fdopen, mock_makedirs, mock_replace, mock_mkstemp):
        ...
        self.assertEqual(call_kwargs.kwargs.get("prefix") or call_kwargs[1].get("prefix"), "scoop-")
